...
export class AppLogger implements LoggerService {
  ...
  logEmailSent(to: string, subject: string, duration: number, context?: LogContext) {
    const message = `Email sent to ${to} - ${subject} (${duration}ms)`;
    console.log(this.formatMessage(LogLevel.INFO, message, context));
  }

  logEmailError(to: string, subject: string, error: string, context?: LogContext) {
    const message = `Email failed to ${to} - ${subject}: ${error}`;
    console.error(this.formatMessage(LogLevel.ERROR, message, context));
  }
  ...
  logValidationError(errors: any[], context?: LogContext) {
    this.error('Validation failed', undefined, {
      ...context,
      validationErrors: errors,
    });
  }

  logBusinessLogicError(operation: string, error: any, context?: LogContext) {
    this.error(`Business logic error in ${operation}`, error?.stack, {
      ...context,
      operation,
      error: error?.message,
    });
